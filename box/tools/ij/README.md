...
- MFA-protected profiles with a single OTP prompt per invocation
...
source profile's credentials, and reuses the resulting temporary credentials
for both region scanning and the spawned `aws ssm start-session` (passed via
`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_SESSION_TOKEN`), so the
AWS CLI does not re-prompt for the OTP.
