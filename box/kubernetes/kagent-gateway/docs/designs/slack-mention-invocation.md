...
thread, and let the conversation continue in that thread. The gateway opens a
...
Today the gateway is one directional. Alertmanager pushes a webhook, an agent
...
- Making the agent act on the cluster. The agent stays read only, enforced by
  the tools bound to it, not by the gateway.
...
1. `apps.connections.open` with the app level token returns a single use `wss`
   URL.
2. The gateway dials it and reads envelopes. `hello` confirms the session,
...
4. The turn runs detached, exactly as `analyze` already runs detached from the
   webhook request.
5. On read error, `disconnect`, or a closed socket, the gateway reopens with
...
- `bot_id` is set, or the author is the gateway's own user id, which is resolved
  once at startup with `auth.test`. This is the loop guard: the gateway posts
...
the gateway never broadcasts its own reply to the channel.
...
names an event the agent has never seen. The gateway therefore quotes the message
...
whole thread in the gateway was tried first: pagination, a rune budget, keeping
...
worked, and all of it was the gateway guessing how much of a conversation a
...
channel as the app, and the gateway already owns everything the bot says.
...
where the gateway's own expiry would have cancelled it with `tasks/cancel` and
...
When no slot frees up before the deadline, the gateway posts a short note in the
...
| `CHAT_TIMEOUT` | `180s` | Deadline for one whole turn, including queueing. Same as the controller's 3 minute cap. Lower it to make the gateway's own expiry fire first and cancel the task. |
...
|--------|------|-------------|
| `kagent_gateway_socket_connected` | Gauge | 1 while a Socket Mode connection is established. |
| `kagent_gateway_socket_connections_total{result}` | Counter | Connection attempts, by `ok`, `error`, or `disconnect_requested`. |
| `kagent_gateway_chat_events_total{result}` | Counter | Mention events, by `accepted` or the drop reason (`bot`, `subtype`, `dm`, `channel_denied`, `user_denied`, `not_in_thread`, `duplicate`, `empty`). |
| `kagent_gateway_chat_turns_total{agent,result}` | Counter | Agent turns, by `ok`, `error`, or `queue_timeout`. |
| `kagent_gateway_chat_turn_duration_seconds` | Histogram | Wall clock duration of one turn. |
| `kagent_gateway_chat_inflight` | Gauge | Turns currently executing. |
| `kagent_gateway_chat_slots` | Gauge | Configured `MAX_CONCURRENT_CHATS`, so saturation reads as a ratio, matching `analysis_slots`. |
| `kagent_gateway_chat_sessions` | Gauge | Threads currently holding a `contextId`. |
