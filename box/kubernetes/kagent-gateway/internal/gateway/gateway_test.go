...
func newTestGateway(t *testing.T, cfg config.Config, sc SlackClient, agent AgentClient) *Gateway {
	t.Helper()
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	g := New(cfg, sc, agent, observability.NewMetrics(), logger)
	g.lookupBackoff = time.Millisecond // keep retry tests fast
	t.Cleanup(func() { g.Wait(3 * time.Second) })
	return g
}

func post(t *testing.T, g *Gateway, body string, headers map[string]string) *httptest.ResponseRecorder {
	t.Helper()
...
func TestWebhookPostsAlertThenAnalysis(t *testing.T) {
	...
	g := newTestGateway(t, testConfig(), sc, agent)

	if rec := post(t, g, criticalAlert, nil); rec.Code != http.StatusAccepted {
...
func TestInvestigatingReactionWrapsAnalysis(t *testing.T) {
	...
	sc, agent := newFakeSlack(), &fakeAgent{reply: "*Summary* OOMKilled"}
	g := newTestGateway(t, cfg, sc, agent)

	if rec := post(t, g, criticalAlert, nil); rec.Code != http.StatusAccepted {
		t.Fatalf("status = %d, want 202", rec.Code)
	}
	msgs := sc.waitFor(t, 2)
	if !g.Wait(3 * time.Second) {
...
func TestWebhookReportsAgentFailureInThread(t *testing.T) {
	sc := newFakeSlack()
	g := newTestGateway(t, testConfig(), sc, &fakeAgent{err: summaryError{
		summary: "bedrock throttled",
		detail:  "submit analysis: agent error -32603: bedrock throttled",
	}})

	post(t, g, criticalAlert, nil)
...
func TestWebhookSkipsUnwantedAlerts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if rec := post(t, g, tt.body, nil); rec.Code != http.StatusAccepted {
				t.Fatalf("status = %d, want 202", rec.Code)
			}
			sc.waitFor(t, 1)
			g.Wait(time.Second)
...
func TestOptInLabelOverridesSeverityFilter(t *testing.T) {
	...
	post(t, g, `{"groupKey":"g","status":"firing",
...
func TestAnalyzeResolvedWhenEnabled(t *testing.T) {
	...
	post(t, g, `{"groupKey":"g","status":"resolved","commonLabels":{"alertname":"A","severity":"critical"},
...
func TestEmptySeverityFilterAnalysesEverything(t *testing.T) {
	...
	post(t, g, `{"groupKey":"g","status":"firing","commonLabels":{"alertname":"A","severity":"info"},
...
// Alertmanager must retry a notification the gateway failed to deliver, which
...
func TestWebhookReturns502WhenSlackFails(t *testing.T) {
	...
	if rec := post(t, g, criticalAlert, nil); rec.Code != http.StatusBadGateway {
...
func TestWebhookRejectsBadInput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sc := newFakeSlack()
			g := newTestGateway(t, testConfig(), sc, &fakeAgent{})

			if rec := post(t, g, tt.body, nil); rec.Code != tt.want {
...
func TestWebhookBearerToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sc := newFakeSlack()
			g := newTestGateway(t, cfg, sc, &fakeAgent{reply: "ok"})

			if rec := post(t, g, criticalAlert, tt.header); rec.Code != tt.want {
				t.Fatalf("status = %d, want %d", rec.Code, tt.want)
			}
		})
	}
...
func TestHandlerServesHealthEndpoints(t *testing.T) {
	g := newTestGateway(t, testConfig(), newFakeSlack(), &fakeAgent{})
...
func TestResolveChannel(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := g.resolveChannel(p); got != tt.want {
...
func TestResolveAgent(t *testing.T) {
	...
	g := newTestGateway(t, cfg, newFakeSlack(), &fakeAgent{})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := g.resolveAgent(decodePayload(t, tt.body)); got != tt.want {
...
// The gateway caps concurrent agent runs so a burst of alerts cannot open an
...
func TestConcurrencyIsCapped(t *testing.T) {
	...
	post(t, g, criticalAlert, nil)
	post(t, g, strings.Replace(criticalAlert, "gk-1", "gk-2", 1), nil)
...
func TestWaitReportsTimeout(t *testing.T) {
	...
	g := New(cfg, sc, agent, observability.NewMetrics(), slog.New(slog.NewTextHandler(io.Discard, nil)))
...
// In lookup mode Alertmanager posts the alert, so the gateway must publish
...
func TestLookupModeOnlyPostsThreadReply(t *testing.T) {
	...
	g := newTestGateway(t, lookupConfig(), sc, agent)
...
// about it, so the gateway has no reason to call the API at all.
func TestLookupModeSkipsSlackEntirelyWhenNotAnalysing(t *testing.T) {
	sc, agent := newFakeSlack(), &fakeAgent{reply: "ok"}
	g := newTestGateway(t, lookupConfig(), sc, agent)

	post(t, g, `{"groupKey":"g","status":"firing","commonLabels":{"alertname":"A","severity":"warning"},
		"alerts":[{"status":"firing","labels":{"severity":"warning"}}]}`, nil)
	g.Wait(time.Second)
...
func TestMarkerFallsBackToGroupKey(t *testing.T) {
	...
	post(t, g, `{"groupKey":"gk-only","status":"firing","commonLabels":{"alertname":"A","severity":"critical"},
...
func TestWebhookCountsIndividualAlerts(t *testing.T) {
	...
	post(t, g, `{"groupKey":"gk-multi","status":"firing",
	...
	for _, tc := range []struct {
		...
		got := metricValue(t, g, "kagent_gateway_alerts_received_total", map[string]string{"severity": tc.severity, "status": tc.status})
		if got != tc.want {
...
func TestTruncatedAnalysisIsCounted(t *testing.T) {
	...
	if got := metricValue(t, g, "kagent_gateway_slack_messages_truncated_total", map[string]string{"kind": "thread"}); got != 1 {
		t.Errorf("slack_messages_truncated_total{kind=\"thread\"} = %v, want 1", got)
	}
	if got := metricValue(t, g, "kagent_gateway_slack_messages_truncated_total", map[string]string{"kind": "parent"}); got != 0 {
		t.Errorf("the short alert body was counted as truncated: %v", got)
	}
}

// The dedupe store is the gateway's only unbounded state, and the gauge is the
...
func TestDedupeGaugeTracksStore(t *testing.T) {
	...
	if got := metricValue(t, g, "kagent_gateway_dedupe_entries", nil); got != 0 {
	...
	post(t, g, criticalAlert, nil)
	sc.waitFor(t, 4)
	if got := metricValue(t, g, "kagent_gateway_dedupe_entries", nil); got != 1 {
...
func TestAnalysisSlotsGaugePublishesTheLimit(t *testing.T) {
	cfg := testConfig()
	cfg.MaxConcurrent = 5
	g := newTestGateway(t, cfg, newFakeSlack(), &fakeAgent{reply: "ok"})

	if got := metricValue(t, g, "kagent_gateway_analysis_slots", nil); got != 5 {
		t.Errorf("analysis_slots = %v, want 5", got)
	}
}

// metricValue reads one series off the gateway's own registry. Gathering rather
...
func metricValue(t *testing.T, g *Gateway, name string, labels map[string]string) float64 {
	t.Helper()
