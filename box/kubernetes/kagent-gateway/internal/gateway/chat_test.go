...
// newChatGateway wires a gateway with the mention path enabled and the loop guard
// pointed at the bot's own user id.
func newChatGateway(t *testing.T, cfg config.Config, sc SlackClient, agent AgentClient) *Gateway {
	t.Helper()
	g := newTestGateway(t, cfg, sc, agent)
	g.SetBotUserID(botUser)
	return g
}
...
func TestMentionAnswersInThread(t *testing.T) {
	...
	g := newChatGateway(t, chatConfig(), sc, agent)

	g.HandleEvent(context.Background(), mention(nil))
...
func TestMentionCarriesTheAlertAndTheThreadIdentifiers(t *testing.T) {
	...
	// itself, which is the whole reason the gateway sends only the parent.
...
func TestMentionCarriesTheAlertOnEveryTurn(t *testing.T) {
	...
	g.HandleEvent(context.Background(), mention(nil))
	g.Wait(3 * time.Second)
	g.HandleEvent(context.Background(), mention(func(ev *socket.Event) { ev.EnvelopeID = "env-2" }))
	g.Wait(3 * time.Second)
...
func TestMentionKeepsMentionsInsideTheQuestion(t *testing.T) {
	...
	g.HandleEvent(context.Background(), mention(func(ev *socket.Event) {
...
func TestMentionStartsFreshAfterTheSessionExpires(t *testing.T) {
	...
	g := newChatGateway(t, cfg, sc, agent)
...
func TestMentionDropRules(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			g.HandleEvent(context.Background(), tc.event)
			g.Wait(time.Second)

			if got := agent.promptCount(); got != 0 {
				t.Fatalf("a dropped mention started %d agent runs, want none", got)
			}
			if got := counterValue(t, g, "chat_events_total", tc.want); got != 1 {
...
func TestMentionAcceptsThreadBroadcast(t *testing.T) {
	...
	g.HandleEvent(context.Background(), mention(func(ev *socket.Event) { ev.Subtype = "thread_broadcast" }))
	g.Wait(3 * time.Second)

	if got := agent.promptCount(); got != 1 {
		t.Fatalf("a thread broadcast started %d agent runs, want 1", got)
	}
	// The gateway answers in the thread and never broadcasts its own reply.
...
func TestMentionIgnoresRedeliveredEnvelope(t *testing.T) {
	...
	if got := counterValue(t, g, "chat_events_total", "duplicate"); got != 1 {
...
func TestMentionHints(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			sc := newFakeSlack()
			g := newChatGateway(t, cfg, sc, &fakeAgent{})
...
func TestMentionHintsCanBeDisabled(t *testing.T) {
	...
	g.HandleEvent(context.Background(), mention(func(ev *socket.Event) { ev.ThreadTS = "" }))
	g.Wait(time.Second)
...
func TestMentionDropsStaySilentWithoutAHint(t *testing.T) {
	sc := newFakeSlack()
	g := newChatGateway(t, chatConfig(), sc, &fakeAgent{})

	g.HandleEvent(context.Background(), mention(func(ev *socket.Event) { ev.User = botUser }))
	g.Wait(time.Second)
...
func TestMentionReportsAgentFailureInThread(t *testing.T) {
	...
	if v := counterValue(t, g, "chat_turns_total", "error"); v != 1 {
...
func TestMentionReportsQueueTimeout(t *testing.T) {
	...
	if v := counterValue(t, g, "chat_turns_total", "queue_timeout"); v != 1 {
...
func TestMentionTruncatesLongReplies(t *testing.T) {
	...
	if v := counterValue(t, g, "slack_messages_truncated_total", "chat"); v != 1 {
...
func counterValue(t *testing.T, g *Gateway, name, result string) float64 {
	t.Helper()
	label := "result"
	if strings.HasSuffix(name, "truncated_total") {
		label = "kind"
	}
	return metricValue(t, g, "kagent_gateway_"+name, map[string]string{label: result})
}
