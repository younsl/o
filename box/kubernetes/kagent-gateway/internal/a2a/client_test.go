...
func newTestClient(t *testing.T, handler http.HandlerFunc) *Client {
	...
	t.Cleanup(srv.Close)
	return New(srv.URL, "kagent", "gateway@kagent.dev", 5*time.Second, 10*time.Millisecond, observability.NewMetrics(), testLogger())
}
...
func TestSendRecordsTaskMetrics(t *testing.T) {
	...
	client := New(srv.URL, "kagent", "gateway@kagent.dev", 5*time.Second, time.Millisecond, metrics, testLogger())
	if _, err := client.Send(context.Background(), Request{Agent: testAgent, Text: "prompt"}); err != nil {
		t.Fatalf("Send() error = %v", err)
	}

	if got := seriesCount(t, metrics, "kagent_gateway_agent_task_duration_seconds", map[string]string{"state": "completed"}); got != 1 {
		t.Errorf("agent_task_duration_seconds_count{state=\"completed\"} = %v, want 1", got)
	}
	if got := seriesSum(t, metrics, "kagent_gateway_agent_task_polls", nil); got != 2 {
		t.Errorf("agent_task_polls_sum = %v, want 2", got)
	}
	if got := seriesCount(t, metrics, "kagent_gateway_agent_request_duration_seconds", map[string]string{"method": "tasks/get"}); got != 2 {
		t.Errorf("agent_request_duration_seconds_count{method=\"tasks/get\"} = %v, want 2", got)
	}
}

// A cancelled analysis never reaches a controller state, so the gateway labels it
...
func TestSendRecordsTimeoutAsATaskState(t *testing.T) {
	...
	client := New(srv.URL, "kagent", "gateway@kagent.dev", 5*time.Second, 5*time.Millisecond, metrics, testLogger())
	...
	if got := seriesCount(t, metrics, "kagent_gateway_agent_task_duration_seconds", map[string]string{"state": "timeout"}); got != 1 {
...
func TestEndpointIsBuiltPerAgent(t *testing.T) {
	...
	client := New(srv.URL+"/", "kagent", "gateway@kagent.dev", 5*time.Second, time.Millisecond, nil, testLogger())
