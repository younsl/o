...
// loop guard: the gateway posts into the channels it listens to. It needs no
// scope of its own.
...
type ThreadMessage struct {
	...
	// notification and a gateway reply are told from a person's own words.
	BotID string
	Text  string
}

// ThreadParent returns the message a thread hangs from, which for an alert
// thread is the alert itself. It is the one piece of a thread the gateway sends
