...
const (
	...
	// ParentModePost makes the gateway publish the alert itself, which yields
...
type Config struct {
	...
	LookupAttempts   int
	// InvestigatingReaction is the emoji (without colons) the gateway puts on
	...
	// so one gateway can feed several specialised agents. Unlike SlackChannelMap
...
func Load() (Config, error) {
	cfg := Config{
		...
		KagentUserID:          getEnv("KAGENT_USER_ID", "gateway@kagent.dev"),
...
// Agents returns every agent the gateway may route to, the default first and
