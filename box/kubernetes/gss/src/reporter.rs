...
mod tests {
    ...
    #[test]
    fn test_convert_cron_to_kst_range() {
        assert_eq!(
            ConsoleFormatter::convert_cron_to_kst("0 9-17 * * *"),
            "0 18-2 * * *"
        );
    }

    #[test]
    fn test_convert_cron_to_kst_invalid_range() {
        // Malformed range with extra dash
        assert_eq!(
            ConsoleFormatter::convert_cron_to_kst("0 9-12-15 * * *"),
            "0 9-12-15 * * *"
        );
    }

    #[test]
    fn test_convert_cron_to_kst_non_numeric() {
        assert_eq!(
            ConsoleFormatter::convert_cron_to_kst("0 abc * * *"),
            "0 abc * * *"
        );
    }
    ...
    fn test_convert_cron_to_kst_invalid_parts() {
        assert_eq!(
            ConsoleFormatter::convert_cron_to_kst("invalid cron"),
            "invalid cron"
        );
    }

    #[test]
    fn test_adjust_day_of_week_single() {
        // increment_day: (d % 7) + 1
        assert_eq!(ConsoleFormatter::increment_day("1"), "2");
        assert_eq!(ConsoleFormatter::increment_day("6"), "7");
        assert_eq!(ConsoleFormatter::increment_day("0"), "1");
    }

    #[test]
    fn test_adjust_day_of_week_comma_separated() {
        assert_eq!(ConsoleFormatter::adjust_day_of_week("1,3,5"), "2,4,6");
    }

    #[test]
    fn test_adjust_day_of_week_range() {
        assert_eq!(ConsoleFormatter::adjust_day_of_week("1-5"), "2-6");
    }

    #[test]
    fn test_adjust_day_of_week_non_numeric() {
        assert_eq!(ConsoleFormatter::increment_day("MON"), "MON");
    }

    #[test]
    fn test_console_formatter_inactive_user() {
        let mut result = ScanResult::new();
        result.total_repos = 3;
        result.scan_duration = Duration::seconds(5);
        ...
        workflow.cron_schedules = vec!["0 20 * * 5".to_string()];
        workflow.last_status = "failure".to_string();
        workflow.workflow_last_author = "departed-dev".to_string();
