...
# Multi-platform Linux build (requires cargo-zigbuild)
make build-all
...
In CI these are exported as environment variables before invoking `cargo zigbuild`, and `build.rs` picks them up automatically.
...
2. **Runtime stage** (`scratch`):
   ...
   - Pre-built binary copied as `/app/filesystem-cleaner`
   - Non-root `USER 65532:65532` (Kubernetes overrides via `securityContext` when UID/GID must match the target container, e.g. `1001` for actions-runner)

Result: Minimal attack surface with a single statically linked binary.
