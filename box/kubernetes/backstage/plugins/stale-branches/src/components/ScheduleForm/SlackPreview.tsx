...
export const SlackPreview = ({
  ...
  <div className="sb-preview-split">
    <div className="sb-preview-pane">
      <Text as="div" variant="body-x-small" color="secondary">
        Message source
      </Text>
      <pre className="sb-settings-preview">{text}</pre>
    </div>
    <div className="sb-preview-pane">
      <Text as="div" variant="body-x-small" color="secondary">
        As Slack renders it{sample ? ', with example data' : ''}
      </Text>
      <div className="sb-slack-card">
        <div className="sb-slack-body">
          {text.split('\n').map((line, index) => (
            <div
              key={`line-${index}`}
              className={line ? 'sb-slack-line' : 'sb-slack-blank'}
            >
