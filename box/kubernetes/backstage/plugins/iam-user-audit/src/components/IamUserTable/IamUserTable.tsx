...
import { MutedUser, WarningDmLog } from '../../api/types';
...
interface IamUserTableProps {
  ...
  mutedMap?: Record<string, MutedUser>;
  onMutedChange?: () => void | Promise<void>;
  overviewFilter?: 'all' | 'muted';
}

export const IamUserTable = ({
  threshold = 90,
  warningDays = 14,
  isAdmin,
  slackUserMap,
  ...
  const handleUnmute = useCallback(
    async (userName: string) => {
      try {
        ...
        // No-op: the next refresh will reconcile state
      }
    },
    [api, onMutedChange],
  );
  ...
  const filteredUsers = useMemo(() => {
    return visibleUsers.filter(u => {
      ...
      const matchesOverview =
        overviewFilter !== 'muted' || Boolean(mutedMap[u.userName]);
      return matchesSearch && matchesConsole && matchesKeyStatus && matchesOverview;
    });
  }, [visibleUsers, searchQuery, consoleFilter, keyStatusFilter, overviewFilter, mutedMap]);
    ...
    <>
          ...
          <Grid.Root columns={{ initial: '1', sm: '2', md: '4' }} gap="3">
            {filteredUsers.map(user => (
              <Grid.Item key={user.userId} className="iam-grid-item">
                <Card className={`iam-card ${getSeverityClass(user.inactiveDays)} ${mutedMap[user.userName] ? 'iam-card-muted' : ''}`}>
                  ...
                  <CardFooter className="iam-card-footer">
                    ...
                    <Flex gap="0" align="center" className="iam-card-footer-actions">
                      {isAdmin && (() => {
                      ...
                      {isAdmin && (() => {
                        const muted = mutedMap[user.userName];
                        if (muted) {
                          return (
                            <TooltipTrigger delay={200}>
                              <ButtonIcon
                                ...
                                aria-label="Unmute alarms"
                                onPress={() => handleUnmute(user.userName)}
                              />
                              <Tooltip style={{ maxWidth: 280 }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 3, fontSize: 12, lineHeight: 1.5 }}>
                                  <div style={{ fontWeight: 700 }}>Muted — click to unmute</div>
                                  <div>By: {muted.mutedBy}</div>
                                  {muted.reason && <div>Reason: {muted.reason}</div>}
                                  <div style={{ opacity: 0.7 }}>Since {new Date(muted.createdAt).toLocaleString()}</div>
                                </div>
                              </Tooltip>
                            </TooltipTrigger>
                          );
                        ...
                        return (
                          <TooltipTrigger delay={200}>
                            <ButtonIcon
                              ...
                              aria-label="Mute alarms"
                              onPress={() =>
                                setMuteTarget({
                                  userName: user.userName,
                                  passwordLastUsed: user.passwordLastUsed,
                                  hasConsoleAccess: user.hasConsoleAccess,
                                })
                              }
                            />
                            <Tooltip>Mute alarms (channel + warning DM)</Tooltip>
                          </TooltipTrigger>
                        );
                      })()}
                    </Flex>
      ...
      {muteTarget && (
        <MuteDialog
          ...
          passwordLastUsed={muteTarget.passwordLastUsed}
          hasConsoleAccess={muteTarget.hasConsoleAccess}
          open
          onClose={() => setMuteTarget(null)}
          onMuted={() => onMutedChange?.()}
        />
      )}
    </>
  );
