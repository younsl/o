...
import { PluginHeader, Container, Box, Button, ButtonIcon, Flex, Tag, TagGroup, Text, Tabs, TabList, Tab, TabPanel, Tooltip, TooltipTrigger } from '@backstage/ui';
import { RiInformationLine, RiShieldUserLine } from '@remixicon/react';
import { iamUserAuditPlugin } from '../../plugin';
import { useApi } from '@backstage/core-plugin-api';
import { useAsyncRetry } from 'react-use';
import { iamUserAuditApiRef } from '../../api';
import { MutedUser, WarningDmLog, SlackHealth } from '../../api/types';
...
export const IamUserAuditPage = () => {
  ...
  const [mutedMap, setMutedMap] = useState<Record<string, MutedUser>>({});
  const [overviewFilter, setOverviewFilter] = useState<'all' | 'muted'>('all');
  ...
  const refreshMutedUsers = useCallback(async () => {
    ...
    try {
      const items = await api.listMutedUsers();
      const map: Record<string, MutedUser> = {};
      for (const m of items) {
        map[m.iamUserName] = m;
      }
      setMutedMap(map);
    } catch {
      // Silent fallback — UI degrades gracefully if listing fails
    }
  }, [api, isAdmin]);
  ...
  return (
    <>
      <PluginHeader
        ...
        customActions={
          <TagGroup>
            <Tag id="plugin-id" size="small">{iamUserAuditPlugin.getId()}</Tag>
          </TagGroup>
        }
      />
      <Container my="4">
        ...
        <Box mt="4" p="3" className="iam-section-box">
          ...
          <div className="iam-summary-bar">
            ...
            {isAdmin && (
              <div
                className={`iam-summary-card iam-summary-clickable ${overviewFilter === 'muted' ? 'iam-summary-active' : ''}`}
                onClick={() =>
                  setOverviewFilter(prev => (prev === 'muted' ? 'all' : 'muted'))
                }
                role="button"
                tabIndex={0}
                onKeyDown={e => {
              ...
              >
                <Text weight="bold" className="iam-summary-value">{mutedCount}</Text>
                ...
                <TooltipTrigger delay={200}>
                  <ButtonIcon
                    ...
                    className="iam-muted-info-btn"
                  />
                  <Tooltip>Users with channel notifications and warning DMs suppressed. Click to filter.</Tooltip>
                </TooltipTrigger>
              </div>
            )}
            {status && (
        ...
        >
          ...
          <TabPanel id="users">
            <IamUserTable
              ...
              mutedMap={mutedMap}
              onMutedChange={refreshMutedUsers}
