...
export const OpenCostPage = () => {
  ...
  const csvTimestamp = () => {
    const now = new Date();
    const d = new Intl.DateTimeFormat('en-CA', { timeZone: billingTz }).format(now).replace(/-/g, '');
    const t = new Intl.DateTimeFormat('en-GB', {
      timeZone: billingTz, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false,
    }).format(now).replace(/:/g, '');
    return `${d}T${t}-${billingTz.replace(/\//g, '-')}`;
  };
  ...
  return (
    <>
      <PluginHeader
        ...
        customActions={
          <TagGroup>
            <Tag id="plugin-id" size="small">{opencostPlugin.getId()}</Tag>
          </TagGroup>
        }
      />
      <Container my="4">
