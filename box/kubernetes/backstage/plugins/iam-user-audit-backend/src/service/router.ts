...
export async function createRouter(options: RouterOptions): Promise<Router> {
  ...
  router.get('/password-reset/admin-status', async (req, res) => {
    ...
    const isGuest = userRef ? parseEntityRef(userRef).name === 'guest' : false;
    res.json({
      isAdmin: !!userRef && (admins.includes(userRef) || isGuest),
    });
  });
  ...
  router.get('/admin/muted-users', async (req, res) => {
    try {
      const userRef = await tryGetUserRef(req);
      if (!isAdminOrGuest(userRef)) {
        res.status(403).json({ error: 'Only admins can view muted users' });
        return;
      }

      const items = await mutedUserStore.list();
      res.json({ items });
    } catch (error) {
      logger.error(`Failed to list muted users: ${error}`);
      res.status(500).json({
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }
  ...
  router.post('/admin/muted-users', async (req, res) => {
    try {
      const userRef = await tryGetUserRef(req);
      if (!isAdminOrGuest(userRef)) {
        res.status(403).json({ error: 'Only admins can mute users' });
        return;
      }
      ...
      logger.info(
        `Muted IAM user ${muted.iamUserName} by ${userRef}${
          muted.reason ? ` (reason: ${muted.reason})` : ''
        }`,
      );
  ...
  router.delete('/admin/muted-users/:userName', async (req, res) => {
    try {
      const userRef = await tryGetUserRef(req);
      if (!isAdminOrGuest(userRef)) {
        res.status(403).json({ error: 'Only admins can unmute users' });
        return;
      }

      const userName = req.params.userName;
      const removed = await mutedUserStore.remove(userName);
      if (!removed) {
        res.status(404).json({ error: `User ${userName} is not muted` });
        return;
      }

      logger.info(`Unmuted IAM user ${userName} by ${userRef}`);
      res.json({ ok: true });
    } catch (error) {
