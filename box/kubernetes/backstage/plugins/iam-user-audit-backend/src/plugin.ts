  ...
  register(env) {
    env.registerInit({
      ...
      async init({
        ...
        const mutedUserStore = await MutedUserStore.create({ database: knex });
          ...
          await scheduler.scheduleTask({
            ...
            fn: async () => {
              try {
                ...
                const muted = await mutedUserStore.listUserNames();
                const inactiveUsers = allUsers.filter(
                  u => u.inactiveDays >= inactiveDays && !muted.has(u.userName),
                );
                if (inactiveUsers.length > 0) {
                  await slackNotifier.notify(inactiveUsers, inactiveDays);
                  logger.info(
                    `Notified Slack about ${inactiveUsers.length} inactive IAM users (${muted.size} muted)`,
                  );
                }
              } catch (error) {
            ...
            fn: async () => {
              try {
                ...
                const candidates = allUsers.filter(
                  u =>
                    u.inactiveDays >= warningThreshold &&
                    !muted.has(u.userName),
                );
