...
- **In-process filtering**: Allow/deny by [event type category](https://docs.aws.amazon.com/health/latest/ug/aws-health-concepts-and-terms.html), AWS service code, region, and `SERVICE/EVENT_TYPE_CODE` pair (e.g. drop `VPN/AWS_VPN_REDUNDANCY_LOSS` while keeping other VPN events, or drop a Direct Connect outage in an unused region while still paging on one at home); the Helm chart manages all filter settings in a ConfigMap and rolls pods on change
...
| ALLOW_REGIONS / DENY_REGIONS | _(empty)_ | Filter by AWS region code (`global` always allowed unless denied) |
