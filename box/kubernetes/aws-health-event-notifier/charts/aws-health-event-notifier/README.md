...
```console
helm install aws-health-event-notifier oci://ghcr.io/younsl/charts/aws-health-event-notifier --version 0.3.0
```
...
```console
helm pull oci://ghcr.io/younsl/charts/aws-health-event-notifier --untar --version 0.3.0
...
| filter.allowRegions | list | `[]` | Allowed AWS region codes (case-insensitive). Empty = allow all. `global` is always allowed unless denied. |
| filter.denyRegions | list | `[]` | Denied AWS region codes (wins over allow). |
