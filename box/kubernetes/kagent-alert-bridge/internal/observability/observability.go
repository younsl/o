...
const namespace = "kagent_alert_bridge"

// Metrics is the bridge metric set, registered on its own registry so the
...
func NewMetrics() *Metrics {
	m := &Metrics{
		...
		AnalysisDuration: prometheus.NewHistogram(prometheus.HistogramOpts{
			...
			Help:      "Wall-clock duration of one polled agent analysis run, measured by the bridge around the whole A2A exchange.",
		...
		AgentTaskDuration: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			...
			Help:      "Time the kagent controller took to drive one task to a terminal state, from the accepted submission to the poll that observed the state, by agent and by that state. The bridge adds two states of its own: timeout when the analysis deadline hit first, and unreachable when polling was abandoned after repeated failures.",
			Buckets:   []float64{1, 5, 10, 20, 30, 60, 90, 120, 180, 300},
		}, []string{"agent", "state"}),
...
// Health serves /healthz and /readyz. The bridge is ready as soon as its
