...
pub async fn run(
    ...
    let alerts_evaluator = match build_alerts(&config.external_url).await {
...
const ALERTS_CONFIGMAP_NAME: &str = "trivy-collector-alerts";

async fn build_alerts(external_url: &str) -> Result<crate::alerts::AlertEvaluator> {
    // Prefer the in-cluster ServiceAccount namespace; fall back to the active
    // kubeconfig context's namespace for local development. Only when both
    // are missing do we land on "default".
    ...
    let namespace =
        std::fs::read_to_string("/var/run/secrets/kubernetes.io/serviceaccount/namespace")
            ...
            .ok()
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| kube_config.default_namespace.clone());
    ...
    let store =
        crate::alerts::AlertStore::new(client, namespace, ALERTS_CONFIGMAP_NAME.to_string());
    ...
    Ok(crate::alerts::AlertEvaluator::with_external_url(store, url))
}
