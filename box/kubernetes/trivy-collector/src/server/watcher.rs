...
use crate::server::api::WatcherStatus;
    ...
    loop {
        tokio::select! {
            ...
            event = stream.next() => {
                match event {
                    Some(Ok(ev)) => {
                        if let Err(e) = handle_vuln_event(&db, &cluster_name, ev, &namespaces, &watcher_status).await {
    ...
    loop {
        tokio::select! {
            ...
            event = stream.next() => {
                match event {
                    Some(Ok(ev)) => {
                        if let Err(e) = handle_sbom_event(&db, &cluster_name, ev, &namespaces, &watcher_status).await {
...
async fn handle_vuln_event(
    ...
    match event {
        ...
        Event::InitDone => {
            ...
            info!("VulnerabilityReport initial sync completed");
        }
    }
...
async fn handle_sbom_event(
    ...
    match event {
        ...
        Event::InitDone => {
            ...
            info!("SbomReport initial sync completed");
        }
    }
