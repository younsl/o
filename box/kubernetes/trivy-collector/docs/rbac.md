...
|------|-------------|----------|
| `role:readonly` | `reports:get`, `clusters:get`, `stats:get`, `tokens:get`, `tokens:create` | Security engineers — view reports, create personal API tokens |
| `role:admin` | `*:*` (all resources, all actions) | Administrators — full access including admin console and destructive operations |
