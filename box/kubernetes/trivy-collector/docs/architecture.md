...
Hub-pull is **always active** in scraper mode; there is no toggle. Behaviour
is controlled by two environment variables:

| Env var | Helm value | Default | Description |
|---|---|---|---|
| `HUB_SECRET_NAMESPACE` | `server.hub.secretNamespace` | pod namespace via Downward API (`fieldRef: metadata.namespace`) | Namespace watched for cluster-registration Secrets. When empty (local dev) the Secret watcher is skipped with a warning and the scraper continues running. |
| `HUB_LABEL_SELECTOR` | `server.hub.labelSelector` | `""` | Additional label filter appended to the default `trivy-collector.io/secret-type=cluster` selector |
...
| `Role` | release namespace only | `secrets` `get / list / watch / create / update / patch / delete` — used for cluster-registration Secret CRUD |
| `RoleBinding` | release namespace only | Binds the above `Role` to the chart ServiceAccount |
