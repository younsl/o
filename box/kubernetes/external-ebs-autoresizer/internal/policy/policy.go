...
type Effective struct {
	...
	// outcomes on matching instances. Only consulted when alerting is
	// globally enabled.
	AlertEnabled          bool
...
func compile(p config.ResizePolicy, defaults Effective) (compiled, error) {
	...
	if rs.AlertEnabled != nil {
		eff.AlertEnabled = *rs.AlertEnabled
	}
...
// Summaries returns one "name(weight=N,paused=B,alert=B)" string per policy in
// file order, so the startup log shows each group's active state, not just its
// existence.
func (r *Resolver) Summaries() []string {
	out := make([]string, len(r.policies))
	for i, c := range r.policies {
		out[i] = fmt.Sprintf("%s(weight=%d,paused=%t,alert=%t)",
			c.policy.Name, c.policy.Weight, c.effective.Paused, c.effective.AlertEnabled)
	}
	return out
}

// AlertPolicyNames splits every policy bucket (the named ones plus the implicit
// default) by its effective alert switch, for startup logging of which groups
// will alert and which are muted.
func (r *Resolver) AlertPolicyNames() (enabled, muted []string) {
	appendTo := func(name string, on bool) {
		if on {
	...
	appendTo(DefaultPolicyName, r.defaults.AlertEnabled)
	for _, c := range r.policies {
		appendTo(c.policy.Name, c.effective.AlertEnabled)
	}
