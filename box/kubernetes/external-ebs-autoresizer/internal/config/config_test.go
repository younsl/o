...
// TestLoadDefaultPolicyAlertEnabled verifies the default-policy alert switch:
// omitted defaults to true, and an explicit false wins over the default.
func TestLoadDefaultPolicyAlertEnabled(t *testing.T) {
	on := writeConfig(t, `
region: r
defaultPolicy:
  usageThresholdPercent: 80
...
`)
	...
	if !c.AlertEnabled {
		t.Error("AlertEnabled = false, want true (default)")
	}
...
`)
	...
	if c.AlertEnabled {
		t.Error("AlertEnabled = true, want false (explicit)")
	}
}
