...
## Per-policy alert switch

On top of the global switch and the notify-on filter, each resize policy can
turn alerting on or off for its own instances with `alertEnabled`:
...
config:
  defaultPolicy:
    ...
    growMode: percent
    # Alert switch for instances matching no named policy. Defaults to true.
    alertEnabled: true
  policies:
    - name: noisy-batch
      instanceSelector:
        nameRegex: "^batch-"
      resize:
        # Mute alerts for this group; every other group keeps alerting.
        alertEnabled: false
```
