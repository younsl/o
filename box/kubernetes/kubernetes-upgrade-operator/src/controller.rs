...
pub struct Context {
    ...
    /// Grafana annotator. `None` when `GRAFANA_ANNOTATION_ENABLED` is not true.
    pub grafana: Option<Arc<Annotator>>,
}
    ...
    match result {
        Ok((mut new_status, requeue)) => {
            ...
            if new_phase != old_phase {
                ...
                // Grafana: mark where each phase begins. This runs inside the
                // transition branch, so exactly one marker is posted per phase
                // no matter how many reconciles that phase takes to finish.
                if let Some(ref annotator) = ctx.grafana {
                    annotator
                        .phase_started(name, spec, &new_status, &new_phase)
                        .await;
                }
            }
            ...
            match new_status.phase {
                Some(UpgradePhase::Completed) => {
                    ...
                    // Grafana: close the run out with a region annotation
                    // spanning startedAt..completedAt, so the whole upgrade
                    // window reads as one shaded band behind the phase markers.
                    if let Some(ref annotator) = ctx.grafana {
                        annotator.run_finished(name, spec, &new_status, None).await;
                    }
                }
                Some(UpgradePhase::Failed) => {
                    ...
                    // Grafana: `old_phase` is the phase that was running when
                    // the handler gave up, which is what the annotation should
                    // name. `new_status.phase` is already `Failed` by now.
                    if let Some(ref annotator) = ctx.grafana {
                        annotator
                            .run_finished(name, spec, &new_status, Some(&old_phase))
                            .await;
                    }
                }
                _ => {}
        ...
        Err(e) => {
            ...
            // Grafana: close the run out, naming the phase that raised the
            // error rather than the `Failed` state it was just put into.
            if let Some(ref annotator) = ctx.grafana {
                annotator
                    .run_finished(name, spec, &new_status, Some(&old_phase))
                    .await;
