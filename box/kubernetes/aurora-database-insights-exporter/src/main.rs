...
use crate::aws::discovery::{AwsRdsDiscoverer, InstanceChannel};
use crate::config::{Args, Config};
use crate::k8s::leader::{LeaderChannel, LeaderElector};
...
async fn main() {
    ...
    // Shared state
    let (instances_tx, instances_rx) = watch::channel(Arc::new(Vec::<AuroraInstance>::new()));
    let instances_tx: Arc<InstanceChannel> = Arc::new(instances_tx);
    ...
    // true if LE disabled. Held here so the channel outlives every receiver.
    let (leader_tx, leader_rx) = watch::channel(!config.leader_election.enabled);
    let leader_tx: Arc<LeaderChannel> = Arc::new(leader_tx);
    ...
    let leader_elector: Option<Arc<LeaderElector>> = if config.leader_election.enabled {
        ...
        match LeaderElector::new(config.leader_election.clone(), leader_tx.clone()).await {
            Ok(le) => {
                let le = Arc::new(le);
                let le_run = le.clone();
...
async fn discovery_loop(
    ...
    loop {
        let leading = *is_leader.borrow();
        if !leading {
            tracing::debug!(
                cycle = cycle + 1,
                region = %region,
                "Skipping discovery because this instance is not the leader, waiting for leadership"
            );
            if is_leader.wait_for(|leader| *leader).await.is_err() {
                tracing::error!("Leader election channel closed. Stopping discovery loop");
                return;
            }
            continue;
        }
        ...
        match aws::discovery::run_discovery_cycle(&*discoverer, &config, &state).await {
            Ok(result) => {
                ...
                // The collection loop wakes on the published list itself, no separate signal.
...
async fn collection_loop_with_leader(
    ...
    mut instances_state: watch::Receiver<Arc<Vec<AuroraInstance>>>,
    region: String,
    ...
    loop {
        ...
        if !leading {
            tracing::debug!(
                cycle = cycle + 1,
                "Skipping collection because this instance is not the leader. Waiting for leadership"
            );
            if is_leader.wait_for(|leader| *leader).await.is_err() {
                tracing::error!("Leader election channel closed. Stopping collection loop");
                return;
            }
            continue;
        }

        let instances = Arc::clone(&instances_state.borrow());
        if instances.is_empty() {
            tracing::debug!(
                cycle = cycle + 1,
                "No instances discovered. Waiting for discovery"
            );
            if instances_state
                .wait_for(|discovered| !discovered.is_empty())
                .await
                .is_err()
            {
                tracing::error!("Discovery channel closed. Stopping collection loop");
                return;
            }
            continue;
        }
