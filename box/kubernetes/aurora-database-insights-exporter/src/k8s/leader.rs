...
/// Broadcasts whether this replica currently holds the lease.
///
/// A watch channel rather than a flag plus a notification: `watch::Receiver::wait_for`
/// re-checks the current value before parking, so a worker that samples the flag just before
/// a leadership transition cannot sleep through the only wakeup it will ever get.
pub type LeaderChannel = watch::Sender<bool>;
    ...
    pub async fn run(&self) {
        loop {
            match self.try_acquire_or_renew().await {
                Ok(true) => {
                    let acquired = self.is_leader.send_if_modified(|leader| {
                        if *leader {
                            false
                        } else {
                            *leader = true;
                ...
                Ok(false) => {
                    let lost = self.is_leader.send_if_modified(|leader| {
                        if *leader {
                            *leader = false;
                            true
                        } else {
