...
/// Shared discovery output. Publishing through a watch channel makes "the list changed" and
/// "the list is non-empty" a single observable state, so a waiting collector cannot miss a
/// wakeup the way a separate notification would.
pub type InstanceChannel = watch::Sender<Arc<Vec<AuroraInstance>>>;

/// Run one discovery cycle: fetch instances from RDS and publish the new list.
pub async fn run_discovery_cycle<D: RdsDiscoverer>(
    discoverer: &D,
    config: &DiscoveryConfig,
    state: &InstanceChannel,
    ...
    let new_ids: std::collections::HashSet<String> = new_instances
        .iter()
        .map(|i| i.dbi_resource_id.clone())
        .collect();

    let current = Arc::clone(&state.borrow());
    ...
    state.send_replace(Arc::new(new_instances));
...
mod tests {
    ...
    async fn test_run_discovery_cycle() {
        ...
        let config = default_discovery_config();
        let (state, _rx) = watch::channel(Arc::new(Vec::new()));
    ...
    async fn test_run_discovery_cycle_detects_removal() {
        let (state, _rx) = watch::channel(Arc::new(vec![AuroraInstance {
        ...
        assert_eq!(state.borrow()[0].db_instance_identifier, "new-writer");
    }
    ...
    async fn test_run_discovery_cycle_failure_retains_state() {
        ...
        assert_eq!(state.borrow().len(), 1);
        assert_eq!(state.borrow()[0].db_instance_identifier, "existing-writer");
    }
