...
/// Upper bound on the `sql_full_text` label. Performance Insights returns statements of
/// arbitrary length, and every byte stays resident in the registry and is re-serialized on
/// each scrape, once per top-SQL entry per instance.
const SQL_FULL_TEXT_MAX_LEN: usize = 1024;
...
mod tests {
    ...
    #[test]
    fn test_truncate_sql_multibyte_boundary() {
        // 100 three-byte characters: the 200-byte cut lands inside a character, and slicing
        // there would panic.
        let input = "한".repeat(100);
        let (text, truncated) = truncate_sql(&input);
        assert!(truncated);
        assert!(text.ends_with("..."));
        // Backs off 200 -> 198 bytes, so 66 whole characters survive.
        assert_eq!(text.trim_end_matches('.').chars().count(), 66);
    }

    #[test]
    fn test_truncate_at_caps_full_text() {
        let input = "x".repeat(SQL_FULL_TEXT_MAX_LEN * 3);
        let (text, truncated) = truncate_at(&input, SQL_FULL_TEXT_MAX_LEN);
        assert!(truncated);
        assert_eq!(text.len(), SQL_FULL_TEXT_MAX_LEN + 3);
    }
    ...
    #[tokio::test]
    async fn test_collect_instance_caps_sql_full_text() {
        let mut pi = MockPiCollector::new();
        pi.sql_keys = vec![DimensionKeyResult {
            dimensions: vec![
                ("db.sql.id".to_string(), "HUGE1".to_string()),
                ("db.sql.statement".to_string(), "SELECT 1".to_string()),
            ],
            value: 1.0,
            additional_metrics: HashMap::new(),
        }];
        pi.sql_full_texts = HashMap::from([("HUGE1".to_string(), "y".repeat(8000))]);
        let instance = test_instance();
        let config = CollectionConfig::default();
        ...
        // The full text becomes a Prometheus label, so it must not carry the raw 8 KB.
        assert_eq!(
            snapshot.top_sql[0].sql_full_text.len(),
            SQL_FULL_TEXT_MAX_LEN + 3
        );
    }
